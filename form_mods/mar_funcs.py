...
def enter_mar_section():
    ...
    d = element_locator('//*[@id="refreshArea"]/div/div/div/div[2]/div[1]/div[3]/h5', "x")
    if "7" in d.text:
        global mode
        mode = "all"
...
def mar_second_page():
    standard_click("#gridCustomerMedication > div.k-pager-wrap.k-grid-pager.k-widget.k-floatwrap > div > ul > li:nth-child(3) > a")
    sleep(5)
    if mode == "all":
...
def mar_third_page():
    # enter third page
    standard_click("#gridCustomerMedication > div.k-pager-wrap.k-grid-pager.k-widget.k-floatwrap > div > ul > li:nth-child(4) > a")
    sleep(5)
    if mode == "all":
